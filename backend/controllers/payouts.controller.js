...
const payoutsController = {
  ...
  async initiate(req, res, next) {
    try {
      ...
      if (activeLoan) {
        loanDeduction = Math.round(netAmount * (Number(activeLoan.repaymentPercent) / 100));
        ...
        const newRepaidAmount = Number(activeLoan.repaidAmount) + loanDeduction;
        await prisma.loan.update({
          where: { id: activeLoan.id },
          data: {
            repaidAmount: BigInt(Math.round(newRepaidAmount)),
      ...
      // Step 8: Create Payout record in DB
      const payout = await prisma.payout.create({
        data: {
          userId,
          status: floatSufficient ? "processing" : "queued",
      ...
      res.json({
        ...
        data: {
          payoutId: payout.id,
          grossAmount: requestedAmount / 100,
          fee: fee / 100,
          loanDeduction: loanDeduction / 100,
          netAmount: finalAmount / 100,
          status: floatSufficient ? "processing" : "queued",
          floatAvailable: floatSufficient,
          stripeTransferId: transferId,
          estimatedArrival: "60 seconds",
          message: `₹${(finalAmount / 100).toFixed(2)} is being sent to your UPI`
        }
      });
